...
function assetAgeRange(
  asset: { ageRange: string | null } | null | undefined,
): string | null {
  if (!asset?.ageRange) return null;
  return asset.ageRange.toLowerCase().replaceAll("_", " ");
}

function characterAgeRange(value: string | null | undefined): string | null {
        ...
        select: {
          ...
          characters: {
            select: {
              ...
              description: true,
              ageRange: true,
              asset: { select: { mimeType: true, data: true, ageRange: true } },
              spriteAsset: {
                select: {
                  mimeType: true,
                  data: true,
    ...
    return {
      ...
      starCharacter: (() => {
        ...
        return {
          ...
          ageRange:
            assetAgeRange(character.spriteAsset) ??
            assetAgeRange(character.asset) ??
            characterAgeRange(character.ageRange),
