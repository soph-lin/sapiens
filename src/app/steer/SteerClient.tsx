...
type AgentName = "curator" | "researcher" | "director" | "writer" | "artist";
...
const CURATOR_PROMPT_STORAGE_KEY = "sapiens:steer:curator-prompt";
...
type StoredRun = {
  ...
  topic: string | null;
  storyConfig: (Partial<DirectorLimits> & Partial<FlourishConfig>) | null;
  progress: unknown;
  usage: unknown;
  error: string | null;
...
export default function SteerClient({ runSlug }: { runSlug?: string }) {
  ...
  const [flourish, setFlourish] = useState<FlourishConfig>(
    DEFAULT_FLOURISH_CONFIG,
  );
  const [debug, setDebug] = useState<ProgressLogEntry[]>([]);
  const [images, setImages] = useState<GeneratedImage[]>([]);
  const [copied, setCopied] = useState<string | null>(null);
  const [runAllRunning, setRunAllRunning] = useState(false);
  const [cachedInputsLoaded, setCachedInputsLoaded] = useState(
    () => viewingRun,
  );
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      try {
        ...
        const curatorPrompt =
          localStorage.getItem(CURATOR_PROMPT_STORAGE_KEY) ?? "";
        setPanels((current) => ({
          ...current,
          curator: { ...current.curator, input: curatorPrompt },
        }));
      } catch {
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(CURATOR_PROMPT_STORAGE_KEY, panels.curator.input);
    } catch {
      // Ignore storage failures such as private-browsing quota restrictions.
    }
  }, [cachedInputsLoaded, panels.curator.input, viewingRun]);

  useEffect(() => {
    if (!runSlug) return;
    let active = true;
    fetch(`/api/voyages/${encodeURIComponent(runSlug)}`)
      .then(async (response) => {
      ...
      .then((run) => {
        if (!active) return;
        setStoredRun(run);
        const curatorOutput = pretty(run.curatorOutput);
        ...
        const writerAssets =
          parsedWriterOutput &&
          typeof parsedWriterOutput === "object" &&
          !Array.isArray(parsedWriterOutput)
            ? (parsedWriterOutput as Record<string, unknown>).need_assets
            : undefined;
        const progress = Array.isArray(run.progress)
          ? run.progress.filter(
              (entry): entry is ProgressLogEntry =>
                typeof entry === "object" && entry !== null,
            )
          : [];
        const parsedCuratorOutput = parseStoredValue(run.curatorOutput);
        const storedIdea =
          parsedCuratorOutput &&
          typeof parsedCuratorOutput === "object" &&
          !Array.isArray(parsedCuratorOutput)
            ? (parsedCuratorOutput as Record<string, unknown>).idea
            : undefined;
        const storedResearcherInput =
          storedIdea &&
          typeof storedIdea === "object" &&
          !Array.isArray(storedIdea)
            ? (storedIdea as Record<string, unknown>).historicalEvent
            : undefined;
        setDirectorLimits((current) => ({
          ...current,
          ...(run.storyConfig ?? {}),
        }));
        setFlourish((current) => ({ ...current, ...(run.storyConfig ?? {}) }));
        setPanels({
          curator: {
            ...emptyPanel(),
            input: run.steering ?? "",
          ...
          researcher: {
            ...emptyPanel(),
            input:
              typeof storedResearcherInput === "string"
                ? storedResearcherInput
                : "",
          ...
          artist: {
            ...emptyPanel(),
            input: pretty(writerAssets ?? parsedWriterOutput),
            output: artistOutput,
          },
        });
        setDebug(progress);
      ...
      .catch((error: unknown) => {
        if (!active) return;
        setStoredRunError(
          error instanceof Error ? error.message : "Could not load run",
        );
      });
    return () => {
      active = false;
      ...
      const process = (chunk: string) => {
        ...
        for (const block of blocks) {
          ...
          if (parsed.event === "progress") {
            const progressEvent = parsed.data as ProgressLogEntry;
            progressLog?.push(progressEvent);
            setDebug((current) => [...current, progressEvent].slice(-100));
            onPersistProgress?.();
            ...
            const assetEvent: ProgressLogEntry = {
              agent,
              phase: "asset",
              kind: "asset",
              type: asset.type,
              ...
              assetId: asset.assetId,
              imageDataUrls:
                asset.imageDataUrls ?? generated.map((image) => image.url),
              frames: asset.frames,
            ...
            if (agent === "curator") {
              const parsed = parseStoredValue(output);
              const idea =
                parsed && typeof parsed === "object" && !Array.isArray(parsed)
                  ? (parsed as Record<string, unknown>).idea
                  : undefined;
              const historicalEvent =
                idea && typeof idea === "object" && !Array.isArray(idea)
                  ? (idea as Record<string, unknown>).historicalEvent
                  : undefined;
              const sourceSearchTerms =
                idea && typeof idea === "object" && !Array.isArray(idea)
                  ? (idea as Record<string, unknown>).sourceSearchTerms
                  : undefined;
              if (
                typeof historicalEvent === "string" &&
                historicalEvent.trim()
              ) {
                update("researcher", {
                  input: JSON.stringify({
                    historicalEvent,
                    sourceSearchTerms:
                      typeof sourceSearchTerms === "string" &&
                      sourceSearchTerms.trim()
                        ? sourceSearchTerms
                        : historicalEvent,
                  }),
                });
  ...
  async function runAll() {
    ...
    const curatorPrompt = panels.curator.input.trim();
    if (!curatorPrompt) {
      toast.error("Error: enter a Curator prompt first!");
      return;
    }
    ...
    setImages([]);
    setDebug([]);
    const runProgress: ProgressLogEntry[] = [];
    let runSlug: string | null = null;
    const steering = curatorPrompt;
    const runOutputs: Record<string, unknown> = {};
    const runUsage: Record<string, unknown> = {};
    ...
    const persistRun = (
      ...
      if (!runSlug) return;
      Object.assign(runOutputs, outputs);
      Object.assign(runUsage, usage ?? {});
      const payload = {
        slug: runSlug,
        progress: progressEntriesForStorage(runProgress),
        outputs: { ...runOutputs },
        usage: Object.keys(runUsage).length ? { ...runUsage } : undefined,
        status,
        error: error ?? null,
      };
    ...
    try {
      const runResponse = await fetch("/api/steer/runs", {
        ...
        body: JSON.stringify({
          steering,
          storyConfig: { ...directorLimits, ...flourish },
        }),
      });
      ...
      if (!runResponse.ok)
        throw new Error(runBody.error || "Could not create run");
      runSlug = runBody.slug ?? null;
      if (!runSlug) throw new Error("Run was created without a slug");
      persistRun();
      ...
      if (!curatorResult) {
        await persistRun({}, "Curator failed");
        return;
      }
      const curatorOutput = JSON.parse(curatorResult.output);
      const curatorIdea = curatorOutput?.idea;
      const researcherInput =
        curatorIdea &&
        typeof curatorIdea === "object" &&
        !Array.isArray(curatorIdea)
          ? JSON.stringify({
              historicalEvent: curatorIdea.historicalEvent,
              sourceSearchTerms: curatorIdea.sourceSearchTerms,
              plotDirection: curatorIdea.plotDirection,
            })
          : undefined;
      if (
        typeof researcherInput !== "string" ||
        !researcherInput.trim() ||
        typeof curatorIdea?.historicalEvent !== "string" ||
        !curatorIdea.historicalEvent.trim()
      ) {
        await persistRun(
          { curator: curatorOutput },
          "Curator did not return a historical event",
        );
        toast.error(
          "Run all stopped: Curator did not return a historical event.",
        );
        return;
      }
      await persistRun({ curator: curatorOutput }, undefined, undefined, {
        curator: curatorResult.usage,
      });
      ...
      if (!researcherResult) {
        await persistRun({ curator: curatorOutput }, "Researcher failed");
        return;
      }
      const researcherOutput = JSON.parse(researcherResult.output);
      await persistRun(
        { curator: curatorOutput, researcher: researcherOutput },
        undefined,
        undefined,
      ...
      const directorResult = await run(
        ...
        runProgress,
        abortController.signal,
        typeof curatorIdea?.plotDirection === "string"
          ? curatorIdea.plotDirection
          : undefined,
        () => {
          persistRun();
        },
      ...
      if (!writerPayload.need_assets) {
        await persistRun(
          ...
          "Writer did not return need_assets",
        );
      ...
      await persistRun({ artist: artistOutput }, undefined, undefined, {
        artist: artistResult.usage,
      });

      const pipelineCompleteEvent = {
        agent: "system",
        phase: "complete",
        message: "Pipeline complete. Saving story to the database…",
      } satisfies ProgressLogEntry;
      runProgress.push(pipelineCompleteEvent);
      setDebug((current) => [...current, pipelineCompleteEvent].slice(-100));
      ...
      const assets = artistResult.assets.filter(
        (asset, index, all) =>
          all.findIndex(
            (candidate) =>
              candidate.type === asset.type &&
              candidate.name === asset.name &&
              candidate.ageRange === asset.ageRange,
          ) === index,
      );
      const saveProgress = [
        ...progressEntriesForStorage(runProgress),
        {
          agent: "system",
          phase: "save",
          message: "Saving story to the database…",
        },
      ];
      const writerStory = JSON.parse(writerResult.output) as {
        dialogue?: unknown;
      };
      ...
      const response = await fetch("/api/stories/finalize", {
        ...
        body: JSON.stringify({
          ...
          // Finalize derives the published story fields from the agent snapshots.
          director,
      ...
      if (!storyWindow) {
        toast.error(
          "New tab blocked! Allow pop-ups for this site in your browser settings, then try again.",
        );
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : "Run failed";
      const stopped =
        message === RUN_STOPPED_MESSAGE || abortController.signal.aborted;
      if (!stopped) await persistRun({}, message);
      const statusMessage = stopped ? "Run stopped." : message;
      setDebug((current) =>
        [
          ...current,
          { agent: "system", phase: "save", message: statusMessage },
        ].slice(-100),
      );
      if (stopped) toast.success(statusMessage);
  ...
  function stopRunAll() {
    ...
    setDebug((current) =>
      [
        ...current,
        { agent: "system", phase: "stop", message: "Stopping run…" },
      ].slice(-100),
    );
  ...
  const panel = (
    ...
    <section className="rounded-xl border border-white/15 bg-slate-950/35 p-5 shadow-2xl shadow-slate-950/20 backdrop-blur-md">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-[.25em] text-cyan-200/80">
            {agent}
          </p>
        ...
        <button
          onClick={() => run(agent)}
          disabled={viewingRun || runAllRunning || panels[agent].running}
          className="inline-flex items-center gap-2 rounded-lg border border-cyan-100/20 bg-cyan-100/10 px-3 py-2 text-xs font-semibold text-cyan-50 transition hover:bg-cyan-100/20 disabled:cursor-wait disabled:grayscale disabled:opacity-30"
        >
          {panels[agent].running ? (
      ...
      <label className="mb-2 block font-mono text-[11px] uppercase tracking-[.12em] text-white/55">
        {label}{" "}
      ...
      <TextArea
        value={panels[agent].input}
        onChange={(value) => update(agent, { input: value })}
        placeholder={placeholder}
        readOnly={viewingRun}
      />
      {panels[agent].error && (
      ...
      {panels[agent].output && (
        <div className="mt-3">
          <div className="mb-2 flex justify-between text-xs text-white/60">
            <span>
              Output
              {panels[agent].tokens !== null
                ? ` (${panels[agent].tokens.toLocaleString()} tokens used)`
                : ""}
            </span>
            <button
              onClick={() => copy(panels[agent].output)}
              className="inline-flex items-center gap-1 text-cyan-200 transition hover:text-white"
            >
              {copied === panels[agent].output ? (
                <Check size={13} />
              ) : (
                <Clipboard size={13} />
              )}{" "}
              {copied === panels[agent].output ? "Copied" : "Copy"}
            </button>
          </div>
          <pre className="max-h-72 overflow-auto rounded-xl border border-white/10 bg-black/35 p-3 text-xs leading-5 text-white/80">
            {panels[agent].output}
          </pre>
        </div>
      )}
        ...
        <div className="mt-4 border-t border-white/10 pt-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {images.map((image, index) => (
              <div
                key={`${image.name}-${image.url.slice(0, 16)}-${index}`}
                className="space-y-2"
              >
                <Image
                  src={image.url}
                ...
                />
                <div className="text-[10px] leading-4 text-cyan-100/70">
                  <span className="mr-1 font-mono uppercase tracking-[.12em] text-cyan-200/45">
                    {image.type}
                  </span>
                  {image.name}
                </div>
    ...
    <main className="relative h-dvh overflow-hidden bg-[#06111f] font-space text-white">
      ...
      <div className="relative z-10 flex h-full min-h-0 flex-col px-5 py-6 md:px-8 lg:px-10">
        <header className="mx-auto mb-6 flex w-full max-w-[1700px] shrink-0 items-start justify-between gap-4">
          <div>
            <p className="font-mono text-[10px] uppercase tracking-[.35em] text-cyan-200/80">
              Sapiens / steer
            </p>
            ...
            <p className="mt-2 max-w-2xl text-sm text-white/60">
              Run one stage at a time. Outputs automatically feed the next
              stage, but every input remains editable.
            </p>
          </div>
          <Link
            href="/voyages"
            aria-label="Open previous voyages"
            title="Previous voyages"
            className="inline-flex items-center gap-2 rounded-full border border-cyan-200/20 bg-cyan-200/10 px-4 py-2 font-mono text-[10px] uppercase tracking-[.2em] text-cyan-100 transition hover:bg-cyan-200/20"
          >
            <Ship size={15} />{" "}
            <span className="hidden sm:inline">Previous voyages</span>
          </Link>
        </header>
        {storedRunLoading && (
          <p className="mx-auto mb-4 w-full max-w-[1700px] rounded-xl border border-cyan-200/15 bg-cyan-100/[.04] px-4 py-3 text-sm text-cyan-100/70">
            Loading saved run…
          </p>
        )}
        {storedRunError && (
          <p className="mx-auto mb-4 w-full max-w-[1700px] rounded-xl border border-rose-200/20 bg-rose-300/10 px-4 py-3 font-mono text-xs text-rose-100">
            Could not load run: {storedRunError}
          </p>
        )}
        <div className="mx-auto grid min-h-0 w-full max-w-[1700px] flex-1 grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(24rem,32vw)]">
          <div className="scrollbar-pill min-h-0 space-y-4 overflow-y-auto pr-1">
            {runAllRunning ? (
              <button
                onClick={stopRunAll}
                disabled={viewingRun}
                className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-red-200/30 bg-red-200/15 px-4 py-3 text-sm font-semibold text-red-50 transition hover:bg-red-200/25"
              >
              ...
              <button
                onClick={runAll}
                disabled={viewingRun}
                className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-cyan-100/25 bg-cyan-100/15 px-4 py-3 text-sm font-semibold text-cyan-50 transition hover:bg-cyan-100/25 disabled:cursor-not-allowed disabled:opacity-30"
              >
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="font-mono text-[10px] uppercase tracking-[.12em] text-white/50">
                  ...
                  <input
                    ...
                    className="mt-2 w-full rounded-lg border border-white/15 bg-slate-950/60 px-3 py-2 text-xs normal-case tracking-normal text-white outline-none focus:border-cyan-200/50"
                  />
                </label>
                <label className="font-mono text-[10px] uppercase tracking-[.12em] text-white/50">
                  Max characters
                  <input
                    ...
                    min={1}
                    max={100}
                    value={directorLimits.maxCharacters}
                    readOnly={viewingRun}
                    onChange={(event) =>
                      setDirectorLimits((current) => ({
                        ...current,
                ...
                <label className="font-mono text-[10px] uppercase tracking-[.12em] text-white/50">
                  ...
                  <input
                    ...
                    min={1}
                    max={10}
                    value={directorLimits.maxTries}
                    readOnly={viewingRun}
                    onChange={(event) =>
                      setDirectorLimits((current) => ({
                        ...current,
                        maxTries: Number(event.target.value),
                      }))
                    }
                    className="mt-2 w-full rounded-lg border border-white/15 bg-slate-950/60 px-3 py-2 text-xs normal-case tracking-normal text-white outline-none focus:border-cyan-200/50"
                  />
                ...
                <label className="font-mono text-[10px] uppercase tracking-[.12em] text-white/50">
                  <span className="inline-flex items-center gap-1">
                    Max output tokens
                    <span
                      title="Only applies to Claude. The OpenAI client does not send a max output token limit."
                      aria-label="Claude only"
                    >
                      <Info size={12} className="text-cyan-200/65" />
                    </span>
                  </span>
                  <input
                    type="number"
                    min={1}
                    max={128000}
                    value={directorLimits.maxOutputTokens}
                    readOnly={viewingRun}
                    onChange={(event) =>
                      setDirectorLimits((current) => ({
                        ...current,
            ...
            {panel(
              ...
              "Curate a story idea",
              "Free-text request",
              "cool, or Europe during the Middle Ages",
            )}
            {panel(
              "researcher",
              "Research facts",
            ...
            {panel(
              ...
              "Plan the adventure",
              "Researcher JSON",
              '{"topic": "...", "articleUrl": "https://en.wikipedia.org/wiki/..."}',
            )}
            {panel(
              "writer",
              "Write dialogue",
              "Director JSON",
              '{"synopsis": {}, "characters": [], "endings": [], "collectible": {}, "scenes": []}',
            )}
            {panel(
              "artist",
              "Artist",
              "Asset needs JSON",
              '{"characters": [{"name": "...", "desc": "..."}], "collectible": {"name": "...", "desc": "..."}}',
            )}
          </div>
          <ProgressLog entries={debug} />
        </div>
      </div>
    </main>
  );
