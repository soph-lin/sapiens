...
import { AlertTriangle, CircleSlash2, Eye, ExternalLink, Flag, LoaderCircle, X } from "lucide-react";
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#02080d] text-white">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 py-10 lg:px-10">
            ...
            <table className="w-full min-w-[980px] text-left">
              ...
              <tbody className="divide-y divide-white/5">
                {!loading && filtered.map((run) => <tr key={run.id} className="group text-xs text-white/65 transition hover:bg-cyan-200/[.04]">
                  ...
                  <td className="px-3 py-4">{run.slug ? <Link href={`/steer/${run.slug}`} aria-label="Open voyage run" className="inline-flex items-center justify-center text-cyan-200/75 hover:text-cyan-50" title="Open voyage run"><ExternalLink size={14} /></Link> : <span className="inline-flex text-amber-200/80" aria-label={`Cannot open this run: missing ${run.missing.join(", ")}.`} onMouseEnter={(event) => { const rect = event.currentTarget.getBoundingClientRect(); setTooltip({ text: `Cannot open this run: missing ${run.missing.join(", ")}.`, left: rect.left, top: rect.top, above: rect.top > window.innerHeight / 2 }); }} onMouseLeave={() => setTooltip(null)}><AlertTriangle size={14} className="shrink-0" /></span>}</td>
                  <td className="px-3 py-4">{run.starCharacter ? <button type="button" aria-label={`View star character ${run.starCharacter.name}`} title={`View star character ${run.starCharacter.name}`} className="inline-flex items-center justify-center text-cyan-200/75 hover:text-cyan-50" onClick={() => setSelectedStar(run.starCharacter)}><Eye size={14} /></button> : <span className="inline-flex text-white/20" aria-label="No star character associated with this story" title="No star character associated with this story"><CircleSlash2 size={14} /></span>}</td>
        ...
        {selectedStar && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-6 py-10 backdrop-blur-sm" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedStar(null); }}>
          <section aria-label={`Star character ${selectedStar.name}`} aria-modal="true" className="relative max-h-full w-full max-w-2xl overflow-y-auto rounded-2xl border border-cyan-200/20 bg-[#071219] p-6 shadow-2xl shadow-black/60" role="dialog">
            <button type="button" aria-label="Close star character" title="Close" className="absolute right-4 top-4 rounded-full p-2 text-white/45 hover:bg-white/10 hover:text-white" onClick={() => setSelectedStar(null)}><X size={16} /></button>
            <p className="font-mono text-[10px] uppercase tracking-[.25em] text-cyan-200/55">Star character</p>
            <h2 className="mt-2 pr-10 font-display text-3xl text-white">{selectedStar.name}</h2>
            <p className="mt-3 text-sm leading-6 text-white/65">{selectedStar.description}</p>
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              {selectedStar.portraitUrl && <div className="rounded-xl border border-white/10 bg-black/25 p-3"><p className="mb-2 font-mono text-[9px] uppercase tracking-[.18em] text-white/35">Character</p><Image src={selectedStar.portraitUrl} alt={selectedStar.name} width={256} height={256} unoptimized className="mx-auto aspect-square w-full rounded-lg object-contain [image-rendering:pixelated]" /></div>}
              <div className="rounded-xl border border-cyan-200/15 bg-cyan-100/[.04] p-3"><p className="mb-2 font-mono text-[9px] uppercase tracking-[.18em] text-cyan-200/55">Top-down sprite</p>{selectedStar.spriteUrl ? <Image src={selectedStar.spriteUrl} alt={`${selectedStar.name} top-down sprite`} width={256} height={256} unoptimized className="mx-auto aspect-square w-full rounded-lg object-contain [image-rendering:pixelated]" /> : <p className="flex min-h-32 items-center justify-center text-center text-xs text-white/35">No sprite associated.</p>}</div>
            </div>
          </section>
        </div>}
      </div>
